...
def print_the_team():
    ...
    print("Connor Walker, 9670289\n")
    print("Nathan Armishaw, 9157191\n")
...
def appear_as_subpart(some_part, goal_part):
    ...
    # convert parts to numpy arrays
    s_arry = np.array(some_part)
    g_arry = np.array(goal_part)
    
    s_size = (np.array)(s_arry.shape)
    g_size = (np.array)(g_arry.shape)
    ...
    # test to evaluate if some_part is bigger than goal_part
    if (s_size[0] > g_size[0]) or (s_size[1] > g_size[1]):
        return False
    
    # iterate through goal_part to see if segments match configuration of 
    # some_part
    for start_row in range(g_size[0]):
        for start_col in range(g_size[1]):
            #determine if vertical and horizontal offsets of starting point
            #in goal_part are withing bounds of some_part
            if (g_size[1]-start_col) < s_size[1]:
                break
            elif (g_size[0] - start_row) < s_size[0]:
                break
            
            #test goal_part within the parameters to see if section matches
            #some_part
            match_flag = 0;
            break_flag = 0;
            for i in range(s_size[0]):
                for j in range(s_size[1]):
                    if s_arry[i,j] == g_arry[start_row+i, start_col+j]:
                        match_flag += 1
                    elif s_arry[i,j] == 0:  #0s are automatic matches
                        match_flag += 1
                    else:
                        break_flag = 1;
                        break
            ...
            if match_flag == (s_size[0]*s_size[1]):
                return True
    return False
